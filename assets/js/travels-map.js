...
(function () {
  ...
  function render(){
    ...
    MAP.innerHTML="";
    var head=el("div","city-map-head"),title=el("div","city-map-head__copy");title.innerHTML="<span>TRAVEL ATLAS</span><strong>去过的地方</strong><small>国家保持留白，到访地点会被点亮</small>";
    ...
    var zoom=1,zoomControls=el("div","city-map-zoom"),zoomOut=el("button"),zoomReset=el("button","city-map-zoom__reset"),zoomIn=el("button");
    function setZoom(next){zoom=Math.max(1,Math.min(3,next));var zoomWidth=width/zoom,zoomHeight=height/zoom;svg.setAttribute("viewBox",((width-zoomWidth)/2)+" "+((height-zoomHeight)/2)+" "+zoomWidth+" "+zoomHeight);zoomOut.disabled=zoom===1;zoomIn.disabled=zoom===3;zoomReset.textContent=zoom+"×";hideTooltip();}
